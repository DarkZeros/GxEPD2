...
void GxEPD2_EPD::init(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration, bool pulldown_rst_mode)
{
  ...
  if (_cs >= 0)
  {
    gpio_set_level((gpio_num_t)_cs, HIGH); // preset (less glitch for any analyzer)
    pinMode(_cs, OUTPUT);
    gpio_set_level((gpio_num_t)_cs, HIGH); // set (needed e.g. for RP2040)
  }
  _reset();
  _pSPIx->begin(); // may steal _rst pin (Waveshare Pico-ePaper-2.9)
  if (_rst >= 0)
  {
    gpio_set_level((gpio_num_t)_rst, HIGH); // preset (less glitch for any analyzer)
    pinMode(_rst, OUTPUT);
    gpio_set_level((gpio_num_t)_rst, HIGH); // set (needed e.g. for RP2040)
  }
  if (_cs >= 0)
  {
    gpio_set_level((gpio_num_t)_cs, HIGH); // preset (less glitch for any analyzer)
  ...
  if (_dc >= 0)
  {
    gpio_set_level((gpio_num_t)_dc, HIGH); // preset (less glitch for any analyzer)
    pinMode(_dc, OUTPUT);
    gpio_set_level((gpio_num_t)_dc, HIGH); // set (needed e.g. for RP2040)
  }
  if (_busy >= 0)
  {
    pinMode(_busy, INPUT);
